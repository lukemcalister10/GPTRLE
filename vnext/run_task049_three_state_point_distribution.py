...
import pandas as pd
from task049_three_state_distribution import build_short_support, compare_unchanged_outputs, generate_three_state_predictions
...
def run(source:Path=DEFAULT_SOURCE,out:Path=DEFAULT_OUT):
    ...
    manifest={
      'task':'TASK-049-three-state-point-distribution','state':'candidate-rejected-evidence','source_task047_dir':str(source),
      ...
      'artifacts':{n:{'path':str(out/n),'rows':int(len(pd.read_csv(out/n))),'sha256':sha(out/n),'bytes':(out/n).stat().st_size} for n in ['vnext_predictions.csv','branch_diagnostics.csv','branch_diagnostics_by_fold_lead.csv','non_uncertainty_output_comparison.csv']}
    }
