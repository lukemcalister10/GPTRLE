"""Fast leakage-safe multi-horizon linear hurdle benchmark."""
...
def pre(draft=False):
 ...
 return ColumnTransformer([('n',Pipeline([('i',SimpleImputer(strategy='median')),('s',StandardScaler())]),nums),('c',Pipeline([('i',SimpleImputer(strategy='most_frequent')),('o',OneHotEncoder(handle_unknown='ignore'))]),CAT)])
def fit(tr,te,h,draft=False):
 P=pre(draft);X=P.fit_transform(tr);Z=P.transform(te);ev=(tr[tc(h,'meaningful_seasons')]>0).astype(int).to_numpy();c=LogisticRegression(C=.35,max_iter=500,class_weight=None).fit(X,ev);p=np.clip(c.predict_proba(Z)[:,1],.002,.998);a=ev==1
 def r(t,log=False,cap=None):
  y=tr.loc[a,t].to_numpy(float);y=np.log1p(y) if log else y;m=Ridge(alpha=12).fit(X[a],y);v=m.predict(Z);v=np.expm1(v) if log else v;return np.clip(v,0,cap)
 g=r(tc(h,'games'),True,23*h);pt=r(tc(h,'points'),True,145*23*h);b=r(tc(h,'best_avg'),False,145);return pd.DataFrame({'p':p,'games':p*g,'points':p*pt,'best':p*b},index=te.index)
def naive(tr,te,h):
...
def score(te,p,h):
 ...
 return o
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--dataset',required=True);ap.add_argument('--out',required=True);q=ap.parse_args();d=pd.read_csv(q.dataset);O=Path(q.out);O.mkdir(parents=True,exist_ok=True);rows=[];preds=[];cfg={1:([2023,2024,2025],2023),3:([2021,2022,2023],2021),5:([2019,2020,2021],2019)}
 for h,(years,start) in cfg.items():
  tr=d[(d.origin_year+h<start)&(d.origin_year>=2008)].copy();te=d[d.origin_year.isin(years)].copy()
  for model in ['naive','linear_hurdle','draft_specialist']:
   ...
   r=score(te,p,h);r.update(horizon=h,model=model);rows.append(r);z=te[['key','player','origin_year','position','pick','tenure',tc(h,'games'),tc(h,'points'),tc(h,'best_avg'),tc(h,'meaningful_seasons')]].copy();z.columns=['key','player','origin_year','position','pick','tenure','actual_games','actual_points','actual_best','actual_meaningful'];z[['pred_p','pred_games','pred_points','pred_best']]=p.values;z['horizon']=h;z['model']=model;preds.append(z)
 M=pd.DataFrame(rows);P=pd.concat(preds,ignore_index=True);M.to_csv(O/'summary_metrics.csv',index=False);P.to_csv(O/'predictions.csv',index=False);sl=[]
 for (h,m),g in P.groupby(['horizon','model']):
  for n,z in [('all',g),('draft_time',g[g.tenure==0]),('early_0_2',g[g.tenure<=2]),('first_round',g[g.pick<=20]),('later_41plus',g[g.pick>=41])]:
   ...
   ev=(z.actual_meaningful>0).astype(int);sl.append({'horizon':h,'model':m,'slice':n,'n':len(z),'actual_event':ev.mean(),'pred_event':z.pred_p.mean(),'brier':brier_score_loss(ev,z.pred_p),'auc':roc_auc_score(ev,z.pred_p),'mae_games':mean_absolute_error(z.actual_games,z.pred_games),'mae_points':mean_absolute_error(z.actual_points,z.pred_points),'mae_best':mean_absolute_error(z.actual_best,z.pred_best)})
