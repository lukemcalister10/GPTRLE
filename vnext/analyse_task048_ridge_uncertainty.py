...
QUANTILES = [("points_q10", .10),("points_q25", .25),("points_q50", .50),("points_q75", .75),("points_q90", .90),("points_q97", .97)]
...
def prior_history(total_games: pd.Series) -> pd.Series:
    ...
    return np.select([games.eq(0), games.lt(50)], ["zero_prior_games", "under_50_prior_games"], default="50_plus_prior_games")
...
def realised_cohort(games: pd.Series, points: pd.Series) -> pd.Series:
    ...
    return np.select([g.eq(0) & p.eq(0), g.between(1,5, inclusive="both") & p.gt(0), g.ge(6)], ["zero_game_zero_point", "one_to_five_positive_points", "meaningful_six_plus"], default="other")
...
def validate_quantiles(df: pd.DataFrame, model: str) -> dict[str, Any]:
    ...
    return {"model": model, "negative_quantile_values": int((vals < -1e-12).sum()), "crossing_rows": int((np.diff(vals, axis=1) < -1e-12).any(axis=1).sum())}
    ...
    for keys, g in frame.groupby(groups, dropna=False) if groups else [((), frame)]:
        ...
        for col, tau in QUANTILES:
            for model, mg in g.groupby("model"):
                rows.append({**common,"model":model,"quantile":col,"tau":tau,"n":len(mg),"pinball_loss":float(pinball(mg.points, mg[col], tau).mean())})
    ...
    for keys, g in frame.groupby(groups, dropna=False) if groups else [((), frame)]:
        ...
        for col,tau in QUANTILES:
            for model, mg in g.groupby("model"):
                ...
                rows.append({**common,"model":model,"quantile":col,"tau":tau,"n":len(mg),"empirical_p_actual_le_predicted_quantile":emp,"signed_calibration_error":emp-tau,"absolute_calibration_error":abs(emp-tau),"tie_rule":"actual_points <= predicted_quantile"})
    return pd.DataFrame(rows)


def interval_coverage(frame: pd.DataFrame, groups: list[str]) -> pd.DataFrame:
    intervals=[("q25_q75", "points_q25","points_q75",.50),("q10_q90","points_q10","points_q90",.80),("q10_q97_asymmetric","points_q10","points_q97",.87)]
    rows=[]
    for keys,g in frame.groupby(groups, dropna=False) if groups else [((),frame)]:
        if not isinstance(keys, tuple): keys=(keys,)
        common=dict(zip(groups,keys))
        for name,lo,hi,nom in intervals:
            for model,mg in g.groupby("model"):
                y=mg.points.astype(float); low=mg[lo].astype(float); high=mg[hi].astype(float)
                cov=float(((y>=low)&(y<=high)).mean())
                rows.append({**common,"model":model,"interval":name,"nominal_coverage":nom,"n":len(mg),"coverage":cov,"coverage_error":cov-nom,"absolute_coverage_error":abs(cov-nom),"mean_width":float((high-low).mean())})
    return pd.DataFrame(rows)


def bootstrap_ci(wide: pd.DataFrame, reps: int=BOOTSTRAP_REPLICATIONS, seed: int=BOOTSTRAP_SEED) -> pd.DataFrame:
    rng=np.random.default_rng(seed); players=np.array(sorted(wide.player_key.unique())); rows=[]
    metrics=[("primary_mean_pinball", None)] + QUANTILES
    per_player=[]
    for p,g in wide.groupby("player_key"):
        rec={"player_key":p}
        for col,tau in QUANTILES:
            rec[col]=float((pinball(g.points, g[f"{col}_task047"], tau)-pinball(g.points, g[f"{col}_task012"], tau)).mean())
        rec["primary_mean_pinball"]=float(np.mean([rec[col] for col,_ in QUANTILES]))
        per_player.append(rec)
    pp=pd.DataFrame(per_player).set_index("player_key")
    for metric,_ in metrics:
        obs=float(pp[metric].mean()); draws=np.empty(reps)
        for i in range(reps):
            sample=rng.choice(players, size=len(players), replace=True)
            draws[i]=float(pp.loc[sample, metric].mean())
        rows.append({"metric":metric,"observed_diff_task047_minus_task012":obs,"bootstrap_seed":seed,"replications":reps,"ci_low":float(np.quantile(draws,.025)),"ci_high":float(np.quantile(draws,.975))})
    return pd.DataFrame(rows)


def build_audit(baseline: pd.DataFrame, candidate: pd.DataFrame, targets: pd.DataFrame, features: pd.DataFrame) -> tuple[dict[str, pd.DataFrame], dict[str, Any]]:
    ...
    overall=summarise_pinball(allp, []); primary=overall.groupby("model").pinball_loss.mean().reset_index(name="primary_mean_pinball_loss")
    ...
    cal_overall=calibration(allp, []); cal_lead=calibration(allp,["lead"]); cal_pos=calibration(allp,["broad_position"]); cal_hist=calibration(allp,["prior_history_cohort"])
    cov_overall=interval_coverage(allp, []); cov_lead=interval_coverage(allp,["lead"]); cov_pos=interval_coverage(allp,["broad_position"]); cov_hist=interval_coverage(allp,["prior_history_cohort"])
    zrows=[]
    ...
    integrity = pd.DataFrame([
        ...
        {"check": "target_points_used_directly", "passed": True, "detail": "joined targets.csv points column without reconstruction"},
        ...
        *[{"check": f"{r['model']}_quantiles_non_crossing", "passed": r["crossing_rows"] == 0, "detail": str(r["crossing_rows"])} for r in qv],
        {"check": "no_model_artifacts_or_fitting", "passed": True, "detail": "analysis-only script reads predictions, targets and features"},
        {"check": "point_forecast_inputs_unchanged_by_audit", "passed": True, "detail": "audit does not write prediction inputs or model artifacts"},
    ])
    # Gates
    p = primary.set_index("model").primary_mean_pinball_loss; primary_diff=(p.task047-p.task012)/p.task012
    boot_primary=boot[boot.metric.eq("primary_mean_pinball")].iloc[0]
    indiv=pinball_overall.copy(); max_indiv=float(indiv.pct_diff_task047_minus_task012.max())
    cand_cal=cal_overall[cal_overall.model.eq("task047")]; mean_abs_cal=float(cand_cal.absolute_calibration_error.mean())
    ...
    onefive_misleading = bool(len(onefive) and (onefive[QCOLS].eq(0).mean().mean() > .50) and (onefive.points.gt(0).all()))
    gate_rows=[
...
def main() -> int:
    p=argparse.ArgumentParser(); p.add_argument("--baseline",type=Path,required=True); p.add_argument("--candidate",type=Path,required=True); p.add_argument("--targets",type=Path,required=True); p.add_argument("--features",type=Path,required=True); p.add_argument("--out",type=Path,required=True); args=p.parse_args()
    tables, summary=build_audit(_load_predictions(args.baseline,"task012"), _load_predictions(args.candidate,"task047"), pd.read_csv(args.targets), pd.read_csv(args.features))
    write_outputs(tables, summary, args.out, {"baseline":args.baseline,"candidate":args.candidate,"targets":args.targets,"features":args.features})
