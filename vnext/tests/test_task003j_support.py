...
from analyse_task003j_support import (
    aggregate_support,
    raw_feature_summary,
    transformed_summary,
)
...
def test_raw_feature_summary_flags_extrapolation_and_unseen_categories():
    reference = frame()
...
def test_transformed_summary_reports_distance_and_norm_exceedance():
    reference = frame(
        position=("MID", "RUC", "DEF"),
        draft_type=("ND", "RD", "ND"),
    )
    artifact = Artifact()
...
def test_aggregate_support_is_weighted_by_probe_rows():
    summary = pd.DataFrame(
        [
            {
                "cohort": "ruck",
                "reference": "meaningful_fit_rows",
                "n_probe": 1,
                "nearest_train_distance_median": 1.0,
                "nearest_train_distance_p90": 2.0,
                "nearest_train_distance_p99": 3.0,
                "probe_norm_above_reference_p99_share": 0.0,
            },
            {
                "cohort": "ruck",
                "reference": "meaningful_fit_rows",
                "n_probe": 3,
                "nearest_train_distance_median": 5.0,
                "nearest_train_distance_p90": 6.0,
                "nearest_train_distance_p99": 7.0,
                "probe_norm_above_reference_p99_share": 1.0,
            },
        ]
    )
    result = aggregate_support(summary).iloc[0]
    assert result["nearest_train_distance_p90"] == 5.0
    assert result["probe_norm_above_reference_p99_share"] == 0.75
