...
TASK-048 is complete as an evaluation-only audit. The script compared TASK-012 and TASK-047 point distributions on the locked 20,094 rows per model, used `targets.csv` `points` directly, used the documented `actual_points <= predicted_quantile` tie rule, and recorded deterministic player-block bootstrap confidence intervals with seed `48047` and 2,000 replications.

Result: TASK-047 uncertainty outputs are **not accepted**. Primary mean pinball loss improved from 149.97 to 141.11 and gates 1 and 2 passed, but gates 3, 4, 5, 6 and 7 failed. The TASK-047 overall mean absolute quantile calibration error was 0.1273, q25-q75 coverage was 0.5877 outside the 0.47-0.53 gate, max lead central-interval absolute coverage error was 0.1663, and max eligible subgroup central-interval absolute coverage error was 0.2540; 1,776 one-to-five-game positive-point rows expose structural incompatibility with the exact non-meaningful zero-mass approximation.

Uncertainty remains blocked. This PR does not repair, recalibrate, blend or change the uncertainty model, and does not promote production or keeper utility. The recommended next task is one isolated TASK-049 hypothesis for a target-compatible uncertainty repair focused on calibration and central interval coverage while preserving the accepted TASK-047 point forecasts unless separately justified.
