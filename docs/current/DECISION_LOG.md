...
TASK-048 is an evaluation-only audit of unchanged TASK-047 point quantiles against the locked `targets.csv` `points` target on the exact 20,094 rows per model and 25 legal folds. The audit improves primary mean pinball loss versus TASK-012, but uncertainty acceptance fails predeclared gates 3, 4, 5 and 6: overall absolute quantile calibration error is too high, q25-q75 coverage is too wide, lead-level central interval coverage has material failures, and broad-position/prior-history subgroup coverage has material failures.
